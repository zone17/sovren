#!/usr/bin/env ts-node
/**
 * IMMED-004: Fully Automated Supabase Credential Rotation with HashiCorp Vault
 *
...
class SupabaseCredentialRotation {
  ...
  private readonly vaultPath = 'supabase/credentials';
  ...
  async rotateCredentials(): Promise<RotationResult> {
    console.log('🔄 Starting zero-downtime Supabase credential rotation with HashiCorp Vault...\n');
    ...
    try {
      ...
      // Step 1: Get current credentials from Vault or environment
      const currentCreds = await this.getCurrentCredentials();
      currentPassword = currentCreds.password;
      await this.backupCredentialsToVault(currentCreds);
      await this.backupCredentials(currentCreds);
      console.log('✅ Current credentials backed up to Vault and encrypted locally');
      ...
      // Step 6: Update HashiCorp Vault with new credentials
      await this.updateVaultCredentials(newPassword, currentCreds);
      console.log('✅ HashiCorp Vault updated with new credentials');
      ...
      // Step 12: Create audit trail in Vault
  ...
  private async getCurrentCredentials(): Promise<any> {
    ...
    try {
      ...
      if (vaultSecret && vaultSecret.password) {
        console.log('📦 Retrieved current credentials from Vault');
        return vaultSecret;
      }
    } catch (error) {
      console.log('⚠️  No credentials found in Vault, checking environment...');
    }
    ...
    throw new Error('Could not retrieve current credentials from Vault or environment');
  }

  private async backupCredentialsToVault(credentials: any): Promise<void> {
    const timestamp = new Date().toISOString();

    // Store backup in Vault with versioning
    ...
    await this.vault.writeSecret(this.vaultPath, {
      ...currentSecret,
      previous_password: credentials.password,
      backup_timestamp: timestamp,
    });
  }

  private async updateVaultCredentials(newPassword: string, currentCreds: any): Promise<void> {
    const timestamp = new Date().toISOString();

    // Store new credentials in Vault with comprehensive metadata
    await this.vault.writeSecret(this.vaultPath, {
      ...
      connection_string: `postgresql://${currentCreds.username}:${newPassword}@${currentCreds.host}:${currentCreds.port}/${currentCreds.database}`,
    });
  ...
  private async verifyConnectionComprehensive(password: string): Promise<boolean> {
    try {
      ...
      const secret = await this.getCurrentCredentials();
  ...
  private async rollbackCredentials(previousPassword: string): Promise<void> {
    try {
      ...
      // Rollback Vault credentials
      const creds = await this.getCurrentCredentials();
      await this.updateVaultCredentials(previousPassword, creds);
  ...
  private async createAuditTrail(success: boolean, newPassword: string | null, errorMessage?: string): Promise<void> {
    ...
    await this.vault.writeSecret(`audit/supabase-rotation-${Date.now()}`, auditLog);
...
- HashiCorp Vault versioning for instant rollback
...
3. Verify Supabase CLI authentication
...
`;
    ...
    try {
      ...
      execSync(
        `gh issue create --repo zone17/sovren --title "${title}" --body "${body}" --label "security,${success ? 'rotation-success' : 'rotation-failure'},vault"`,
        { stdio: 'pipe' }
      );
    } catch (error) {
      console.warn('⚠️  Could not create GitHub issue:', error);
...
async function main() {
  ...
  if (result.verificationPassed) {
    ...
    console.log('🏛️  Credentials stored in HashiCorp Vault');
    process.exit(0);
  } else {
