...
const EPIC_004_STORIES = [
  // PHASE 1: Audit & Guidelines (Stories 1-5) - Sequential
  {
    ...
    dependencies: [],
    subtasks: [
      { order: 1, description: 'Analyze all existing Redux slices in store configuration', status: 'completed' },
      { order: 2, description: 'Categorize each slice property as client-state, server-state, or mixed', status: 'completed' },
      { order: 3, description: 'Generate JSON audit report with slice categorization', status: 'completed' },
      ...
      { order: 7, description: 'Document migration priorities and complexity', status: 'completed' },
      { order: 8, description: 'Get technical lead approval on audit findings', status: 'completed' }
    ]
  },
  {
    story_id: 'US-E4-002',
    ...
    dependencies: [],
    subtasks: [
      { order: 1, description: 'Find all useQuery, useMutation, useInfiniteQuery hooks in codebase', status: 'completed' },
      { order: 2, description: 'Document each hook with cache key and data type', status: 'completed' },
      { order: 3, description: 'Identify duplicate state between Redux and React Query', status: 'completed' },
      { order: 4, description: 'Generate JSON audit report with query documentation', status: 'completed' },
      { order: 5, description: 'Identify missing React Query hooks for server data', status: 'completed' },
  ...
  },
  {
    ...
    subtasks: [
      { order: 1, description: 'Design decision flow diagram for state selection', status: 'completed' },
  ...
  },
  {
    ...
    subtasks: [
      { order: 1, description: 'Create overall architecture diagram (current vs target)', status: 'completed' },
      { order: 2, description: 'Create data flow diagram (API → React Query → Components)', status: 'completed' },
  ...
  // PHASE 2: Server Data Migration (Stories 6-12) - Can parallelize
  {
    ...
    subtasks: [
      { order: 1, description: 'Create packages/frontend/src/queries/creators/ directory', status: 'pending' },
      { order: 2, description: 'Implement useCreators hook with pagination and filters', status: 'pending' },
      { order: 3, description: 'Implement useCreatorProfile hook for individual creators', status: 'pending' },
      ...
      { order: 8, description: 'Configure appropriate cache times (staleTime, cacheTime)', status: 'pending' },
  ...
  },
  {
    ...
    dependencies: ['US-E4-005'],
    subtasks: [
      { order: 1, description: 'Create packages/frontend/src/queries/content/ directory', status: 'pending' },
      { order: 2, description: 'Implement useContent hook with infinite scrolling', status: 'pending' },
      { order: 3, description: 'Implement useContentItem hook for single content', status: 'pending' },
      { order: 4, description: 'Implement useCreateContent mutation with optimistic updates', status: 'pending' },
      ...
      { order: 7, description: 'Implement useContentStream for real-time updates', status: 'pending' },
  ...
  },
  {
    ...
    dependencies: ['US-E4-005'],
    subtasks: [
      { order: 1, description: 'Create packages/frontend/src/queries/payments/ directory', status: 'pending' },
      ...
      { order: 4, description: 'Implement usePaymentStatus hook with polling logic', status: 'pending' },
  ...
  },
  {
    ...
    subtasks: [
      { order: 1, description: 'Delete creatorsSlice.ts entirely (all server data)', status: 'pending' },
      { order: 2, description: 'Remove server data from cmsSlice.ts (keep only UI state)', status: 'pending' },
      { order: 3, description: 'Remove server data from analyticsSlice.ts', status: 'pending' },
      { order: 4, description: 'Remove all Redux actions for server data fetching', status: 'pending' },
      { order: 5, description: 'Remove all Redux selectors for server data', status: 'pending' },
      { order: 6, description: 'Update store configuration (remove deleted slices)', status: 'pending' },
      { order: 7, description: 'Fix all TypeScript errors from removed exports', status: 'pending' },
      { order: 8, description: 'Update all tests to remove server data expectations', status: 'pending' },
      { order: 9, description: 'Verify no server data remains in Redux (audit)', status: 'pending' },
  ...
  },
  {
    ...
    subtasks: [
      { order: 1, description: 'Update CreatorList component to use useCreators hook', status: 'pending' },
      { order: 2, description: 'Update CreatorProfile component to use useCreatorProfile hook', status: 'pending' },
      { order: 3, description: 'Update ContentList component to use useContent hook', status: 'pending' },
      { order: 4, description: 'Update ContentEditor component to use useContentItem hook', status: 'pending' },
      { order: 5, description: 'Update PaymentHistory component to use useInvoices hook', status: 'pending' },
      { order: 6, description: 'Update SubscriptionManager component to use useSubscriptions hook', status: 'pending' },
      { order: 7, description: 'Update all imports (remove Redux imports, add React Query)', status: 'pending' },
      { order: 8, description: 'Update PropTypes/interfaces for new data shapes', status: 'pending' },
      { order: 9, description: 'Update component tests to use React Query test utils', status: 'pending' },
      { order: 10, description: 'Verify all components render correctly with new hooks', status: 'pending' }
    ]
  },
  {
    story_id: 'US-E4-011',
    ...
    subtasks: [
      { order: 1, description: 'Create packages/frontend/src/queries/queryClient.ts', status: 'pending' },
      { order: 2, description: 'Configure global QueryClient defaults (staleTime, cacheTime, retry)', status: 'pending' },
      { order: 3, description: 'Define cache strategies for creators (5 min staleTime)', status: 'pending' },
      { order: 4, description: 'Define cache strategies for content (1 min staleTime)', status: 'pending' },
      { order: 5, description: 'Define cache strategies for payments (10 min staleTime)', status: 'pending' },
  ...
  },
  {
    ...
    subtasks: [
      { order: 1, description: 'Create packages/frontend/src/queries/errorHandling.ts', status: 'pending' },
      { order: 2, description: 'Implement global query error handler with toast notifications', status: 'pending' },
      { order: 3, description: 'Implement mutation error handler with rollback logic', status: 'pending' },
      { order: 4, description: 'Create QueryErrorBoundary component', status: 'pending' },
      { order: 5, description: 'Add error boundaries to components using queries', status: 'pending' },
      { order: 6, description: 'Implement retry UI components for failed queries', status: 'pending' },
      ...
      { order: 9, description: 'Define user-friendly error messages for common errors', status: 'pending' },
      { order: 10, description: 'Test error scenarios (network failures, 401, 500, etc.)', status: 'pending' }
    ]
  },
  ...
  // PHASE 3: Client State Consolidation (Stories 13-17) - Can parallelize with Phase 2
  {
    ...
    dependencies: ['US-E4-005'],
    subtasks: [
      { order: 1, description: 'Expand uiSlice.ts with all UI state properties', status: 'pending' },
      ...
      { order: 10, description: 'Write unit tests for UI reducers and selectors', status: 'pending' }
    ]
  },
  {
    story_id: 'US-E4-014',
    ...
    subtasks: [
      ...
      { order: 2, description: 'Identify all instances of UI state in query cache', status: 'pending' },
      ...
      { order: 5, description: 'Update components to dispatch Redux actions for UI updates', status: 'pending' },
      { order: 6, description: 'Remove setQueryData calls for UI preferences', status: 'pending' },
      { order: 7, description: 'Verify React Query DevTools shows only server data', status: 'pending' },
  ...
  },
  {
    ...
    subtasks: [
      { order: 1, description: 'Create NotificationCenter component using Redux', status: 'pending' },
      { order: 2, description: 'Implement auto-dismiss timer for non-persistent notifications', status: 'pending' },
      { order: 3, description: 'Implement notification stacking UI', status: 'pending' },
      { order: 4, description: 'Create useNotification hook for easy dispatching', status: 'pending' },
      ...
      { order: 7, description: 'Add accessibility announcements (ARIA live region)', status: 'pending' },
      { order: 8, description: 'Style notifications with different severity levels', status: 'pending' },
      { order: 9, description: 'Write unit tests for notification logic', status: 'pending' },
      { order: 10, description: 'Write integration tests for notification flows', status: 'pending' }
    ]
  },
  {
    story_id: 'US-E4-017',
    ...
    subtasks: [
      ...
      { order: 2, description: 'Define when to use Redux vs local state for forms', status: 'pending' },
      ...
      { order: 6, description: 'Implement step navigation actions (next, previous)', status: 'pending' },
  ...
  // PHASE 4: Testing & Validation (Stories 18-22) - After Phase 2 & 3
  {
    ...
    subtasks: [
      { order: 1, description: 'Set up integration test infrastructure for state management', status: 'pending' },
      { order: 2, description: 'Write React Query data flow tests (fetch and cache)', status: 'pending' },
      { order: 3, description: 'Write Redux state flow tests (actions and selectors)', status: 'pending' },
      ...
      { order: 8, description: 'Achieve 80%+ test coverage for state management', status: 'pending' },
  ...
  },
  {
    ...
    subtasks: [
      { order: 1, description: 'Create benchmark script for state management performance', status: 'pending' },
  ...
  },
  {
    ...
    subtasks: [
      ...
      { order: 5, description: 'Create dashboard for cache metrics visualization', status: 'pending' },
  ...
  },
  {
    ...
    subtasks: [
      ...
      { order: 2, description: 'Write E2E test for creator flow with React Query', status: 'pending' },
      ...
      { order: 5, description: 'Write E2E test for UI state persistence (theme, modals)', status: 'pending' },
  ...
  },
  {
    ...
    subtasks: [
      ...
      { order: 4, description: 'Write Consequences section (positive and negative)', status: 'pending' },
