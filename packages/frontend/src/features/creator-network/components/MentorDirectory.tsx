...
const MentorDirectory: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-gray-900">Mentor Directory</h2>
        <p className="text-sm text-gray-500">Find experienced creators to guide your growth</p>
      </div>
      ...
      {isLoading ? (
        ...
        <ul className="space-y-4" role="list">
          {mentors.map((mentor) => (
            <li key={mentor.id} className="rounded-lg border bg-white p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="font-medium text-gray-900">{mentor.niche}</span>
                ...
                {requestingId === mentor.id ? (
                  <div className="flex flex-col gap-2 min-w-[200px]">
                    <textarea
                      ...
                      rows={3}
                      className="rounded-md border border-gray-300 px-3 py-2 text-sm resize-none focus:border-indigo-500 focus:outline-none"
