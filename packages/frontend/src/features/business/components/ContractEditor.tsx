...
const ContractEditor: React.FC<ContractEditorProps> = ({ templateId, onSaved, onCancel }) => {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Counterparty */}
      <div>
        <label htmlFor="counterparty" className="block text-sm font-medium text-foreground">
          Counterparty Name
        </label>
        <input
          id="counterparty"
      ...
      <div>
        <label htmlFor="contract-text" className="block text-sm font-medium text-foreground">
          Contract Text
        </label>
        ...
        <textarea
          id="contract-text"
          rows={16}
          className="mt-1 block w-full rounded-md border border-border px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500"
          value={filledText}
          onChange={(e) => {
