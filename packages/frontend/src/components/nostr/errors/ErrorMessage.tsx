...
const SeverityIcon: React.FC<{ severity: ErrorSeverity }> = ({ severity }) => {
  ...
  switch (severity) {
    ...
    case ErrorSeverity.CRITICAL:
      return (
        <svg
          className={`${iconClasses} text-red-700`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
          aria-hidden="true"
...
export const ErrorMessage: React.FC<ErrorMessageProps> = ({
  ...
  return (
    <div className={containerClasses} role="alert" aria-live="polite" aria-atomic="true">
      <div className="flex items-start">
        ...
        <div className="ml-3 flex-1">
          {/* Title */}
          <h3 className={titleClasses}>{error.title}</h3>

          {/* Message */}
          <p className={messageClasses}>{error.message}</p>

          {/* Error Code */}
          {showErrorCode && (
            <p className={`${messageClasses} font-mono mt-1`}>Code: {error.code}</p>
          )}
          ...
          {/* Relay Info */}
          {error.relay && <p className={`${messageClasses} text-xs mt-1`}>Relay: {error.relay}</p>}
          ...
          {!compact &&
            showTroubleshooting &&
            error.troubleshootingHints &&
            error.troubleshootingHints.length > 0 && (
              <div className="mt-3">
                <h4 className={`${titleClasses} text-xs mb-1`}>Troubleshooting:</h4>
          ...
          {!compact &&
            ...
            error.recoverySuggestions.length > 0 && (
              <div className="mt-3">
                <h4 className={`${titleClasses} text-xs mb-1`}>What to try:</h4>
                <ul className={`${messageClasses} space-y-0.5 list-disc list-inside`}>
