...
export const EngagementAnalyticsDashboard: React.FC = () => {
  ...
  const metricCards = useMemo<MetricCard[]>(() => {
    ...
    const previousMetrics = ((metrics as Record<string, unknown>).previousPeriod ??
      {}) as typeof currentMetrics;
    ...
    return patterns
      ...
      .flatMap(pattern =>
        pattern.timestamps.map((timestamp, index) => ({
          ...
          pattern: (pattern as Record<string, unknown>).pattern_name as string,
        }))
      )
  ...
  const handleFilterChange = useCallback(
    (key: keyof DashboardFilters, value: DashboardFilters[keyof DashboardFilters]) => {
                    ...
                    <Card key={pattern.pattern_id}>
                      <CardHeader>
                        <div className='flex items-center justify-between'>
                          <CardTitle>
                            {(pattern as Record<string, unknown>).pattern_name as string}
                          </CardTitle>
                          <Badge
