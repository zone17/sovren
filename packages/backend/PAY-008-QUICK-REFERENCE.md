...
await stateMachine.transition(paymentId, PaymentState.PROCESSING, { provider: 'lightning' });
```

### Check If Transition Allowed

```typescript
const canProcess = await stateMachine.canTransition(paymentId, PaymentState.PROCESSING);
```
