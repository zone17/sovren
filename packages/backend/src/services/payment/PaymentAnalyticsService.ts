...
import type { PaymentTransaction, PaymentHistoryQuery } from '../../types/payment';
...
export class PaymentAnalyticsService implements IPaymentAnalyticsService {
  ...
  private realtimeSubscriptions = new Map<
    string,
    (metrics: RealtimeDashboardMetrics) => void | Promise<void>
  >();
  private eventSubscriptions = new Map<
    string,
    (event: PaymentAnalyticsEvent) => void | Promise<void>
  >();
    ...
    return this.executeCachedQuery(
      ...
      async () => {
        ...
        const periodDuration =
          (query.endDate?.getTime() || Date.now()) - (query.startDate?.getTime() || 0);
        const previousStartDate = new Date(
          (query.startDate?.getTime() || Date.now()) - periodDuration
        );
  ...
  async getRevenueTrend(
    ...
    return this.executeCachedQuery(
      ...
      async () => {
        ...
        const cagr = firstValue > 0 ? (Math.pow(lastValue / firstValue, 1 / periods) - 1) * 100 : 0;
    ...
    return this.executeCachedQuery(
      ...
      async () => {
        ...
        const retrySuccessRate =
          retriedPayments > 0 ? (successAfterRetry / retriedPayments) * 100 : 0;
      ...
      async () => {
        ...
        for (const tx of transactions.filter(t => t.status === 'completed')) {
          ...
          transactionCountByCurrency.set(
            currency,
            (transactionCountByCurrency.get(currency) || 0) + 1
          );
          volumeByCurrency.set(
            currency,
            (volumeByCurrency.get(currency) || 0) + (tx.amountFiat || tx.amount)
          );

          // Convert to base currency
          if (currency === this.baseCurrency) {
            volumeInBaseCurrency.set(
              currency,
              (volumeInBaseCurrency.get(currency) || 0) + tx.amount
            );
          } else {
            const converted = await this.currencyService.convert({
            ...
            volumeInBaseCurrency.set(
              currency,
              (volumeInBaseCurrency.get(currency) || 0) + converted.convertedAmount
            );
          }
        }
    ...
    return this.executeCachedQuery(
      ...
      async () => {
        ...
        for (const method of methods) {
          ...
          const successRate =
            methodTxs.length > 0 ? (successfulTxs.length / methodTxs.length) * 100 : 0;
          const averageFee =
        ...
        const lightningSuccessRate =
          lightningTxs.length > 0 ? (lightningSuccessful.length / lightningTxs.length) * 100 : 0;
        const averageLightningFee =
          lightningSuccessful.length > 0
            ? lightningSuccessful.reduce((sum, t) => sum + (t.fee || 0), 0) /
              lightningSuccessful.length
            : 0;
        ...
        const onchainSuccessRate =
          onchainTxs.length > 0 ? (onchainSuccessful.length / onchainTxs.length) * 100 : 0;
        const averageOnchainFee =
          onchainSuccessful.length > 0
            ? onchainSuccessful.reduce((sum, t) => sum + (t.fee || 0), 0) / onchainSuccessful.length
            : 0;

        // Preferred method
        const sortedMethods = Array.from(methodBreakdown.entries()).sort(
  ...
  async getRevenueByCurrency(currency: Currency, query: AnalyticsQuery): Promise<number> {
    const transactions = await this.getTransactionsForQuery({
      ...query,
        ...
        for (const tx of transactions.filter(t => t.status === 'completed')) {
          ...
          userMetrics.set(tx.userId, {
            ...
            firstTransaction: new Date(
              Math.min(existing.firstTransaction.getTime(), new Date(tx.createdAt).getTime())
            ),
            lastTransaction: new Date(
              Math.max(existing.lastTransaction.getTime(), new Date(tx.createdAt).getTime())
            ),
          });
        }
    ...
    return this.executeCachedQuery(
      ...
      async () => {
        ...
        const arpuGrowthRate =
          previousPeriodArpu > 0 ? ((arpu - previousPeriodArpu) / previousPeriodArpu) * 100 : 0;
        ...
        const medianRevenuePerUser =
          revenuePerUser.length > 0 ? revenuePerUser[Math.floor(revenuePerUser.length / 2)] : 0;
    ...
    return this.executeCachedQuery(
      ...
      async () => {
        ...
        const averageLifetimeValue =
          ltvValues.length > 0 ? totalLifetimeValue / ltvValues.length : 0;
        const medianLifetimeValue =
          ltvValues.length > 0 ? ltvValues[Math.floor(ltvValues.length / 2)] : 0;
  ...
  async getRevenueBreakdownInBaseCurrency(query: AnalyticsQuery): Promise<Map<Currency, number>> {
  ...
  async comparePeriods(
    ...
    const revenueGrowth = period1.netRevenue > 0 ? (revenueDelta / period1.netRevenue) * 100 : 0;

    const transactionDelta = period2.transactionCount - period1.transactionCount;
    const transactionGrowth =
      period1.transactionCount > 0 ? (transactionDelta / period1.transactionCount) * 100 : 0;
  ...
  private forecastRevenue(timeSeries: TimeSeriesDataPoint[], days: number): TimeSeriesDataPoint[] {
    ...
    const dailyChange =
      ((trend.strength * (trend.direction === 'increasing' ? 1 : -1)) / 100) * lastValue;
