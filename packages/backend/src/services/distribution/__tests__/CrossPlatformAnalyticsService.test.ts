...
describe('CrossPlatformAnalyticsService', () => {
  ...
  describe('snapshotMetrics', () => {
    it('should snapshot metrics for connected platforms', async () => {
      mockPlatformService.getStatus.mockResolvedValue([
        { platform: 'mastodon', connected: true },
      ...
      mockPlatformService.getAdapter.mockReturnValue(mockAdapter);

      await service.snapshotMetrics(creatorId);

      expect(mockPlatformService.getAdapter).toHaveBeenCalledWith('mastodon');
      expect(mockPlatformService.getAdapter).not.toHaveBeenCalledWith('twitter');
      expect(mockDb.upsert).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ onConflict: 'creator_id,platform,recorded_at' })
      );
    });

    it('should log error and continue on adapter failure', async () => {
      mockPlatformService.getStatus.mockResolvedValue([
        { platform: 'mastodon', connected: true },
        { platform: 'bluesky', connected: true },
      ]);

      const failAdapter = {
        getMetrics: jest.fn().mockRejectedValue(new Error('API error')),
      };
      ...
      mockPlatformService.getAdapter
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        '[CrossPlatformAnalyticsService] Metrics snapshot failed',
        expect.objectContaining({ platform: 'mastodon' })
      );
      // Should still process bluesky
      expect(mockDb.upsert).toHaveBeenCalledTimes(1);
    });

    it('should skip disconnected platforms', async () => {
      mockPlatformService.getStatus.mockResolvedValue([
        { platform: 'twitter', connected: false },
      ]);
