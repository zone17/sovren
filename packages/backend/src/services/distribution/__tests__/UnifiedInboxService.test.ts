...
describe('UnifiedInboxService', () => {
  ...
  describe('pollMessages', () => {
    it('should poll connected platforms and upsert new messages', async () => {
      ...
      // Mock last fetch query
      mockDb.single.mockResolvedValue({ data: { fetched_at: '2026-02-16T12:00:00Z' } });

      const totalNew = await service.pollMessages(creatorId);
    ...
    it('should log error and continue on platform failure', async () => {
      mockPlatformService.getStatus.mockResolvedValue([
        { platform: 'mastodon', connected: true },
      ]);
      mockPlatformService.getAdapter.mockImplementation(() => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith(
        '[UnifiedInboxService] Poll failed for platform',
        expect.objectContaining({ platform: 'mastodon' })
      );
    });

    it('should return 0 when no connected platforms exist', async () => {
      mockPlatformService.getStatus.mockResolvedValue([]);
