/**
 * 🎭 CREATOR RECOMMENDATIONS API ROUTES
...
const heavyOperationRateLimit = rateLimit({
  ...
  message: 'Too many intensive operations from this IP, please try again later.',
  standardHeaders: true,
  legacyHeaders: false,
});
...
router.post(
  ...
  [
    ...
    body('primaryTopics').optional().isArray(),
    body('contentStyle').optional().isIn(Object.values(ContentStyle)),
    body('audienceLevel').optional().isIn(Object.values(AudienceLevel)),
  ],
...
router.post(
  ...
  [
    body('targetCreatorId').isUUID().withMessage('Target creator ID must be a valid UUID'),
    body('maxMatches').optional().isInt({ min: 1, max: 50 }),
    body('minSimilarityThreshold').optional().isFloat({ min: 0, max: 1 }),
...
router.post(
  ...
  [
    ...
    body('maxSuggestions').optional().isInt({ min: 1, max: 50 }),
    body('minExpertiseLevel').optional().isFloat({ min: 0, max: 1 }),
    body('contentTypes').optional().isArray(),
    body('excludeFollowed').optional().isBoolean(),
    body('diversityWeight').optional().isFloat({ min: 0, max: 1 }),
  ],
  handleValidationErrors,
  async (req: Request, res: Response, next: NextFunction) => {
    try {
...
router.post(
  ...
  requireAuth,
  recommendationRateLimit,
  [body('discoveryMethod').isIn(Object.values(DiscoveryMethod))],
  handleValidationErrors,
  async (req: Request, res: Response, next: NextFunction) => {
    try {
...
router.post(
  ...
  [
    body('userId').isUUID().withMessage('User ID must be a valid UUID'),
    body('discoveryMethod').optional().isIn(Object.values(DiscoveryMethod)),
    body('searchQuery').optional().isString().isLength({ max: 200 }),
    body('categories').optional().isArray(),
    body('interests').optional().isArray(),
    body('contentTypes').optional().isArray(),
    body('audienceLevel').optional().isIn(Object.values(AudienceLevel)),
    body('verificationStatus').optional().isArray(),
    body('usePersonalization').optional().isBoolean(),
    body('diversityWeight').optional().isFloat({ min: 0, max: 1 }),
    body('noveltyWeight').optional().isFloat({ min: 0, max: 1 }),
    body('page').optional().isInt({ min: 1 }),
    body('limit').optional().isInt({ min: 1, max: 50 }),
    body('sortBy').optional().isIn(['relevance', 'popularity', 'recent', 'engagement']),
...
router.post(
  ...
  [
    ...
    body('algorithm').optional().isIn(Object.values(RecommendationAlgorithm)),
    body('maxRecommendations').optional().isInt({ min: 1, max: 50 }),
    body('minScore').optional().isFloat({ min: 0, max: 1 }),
    body('includeExplanations').optional().isBoolean(),
    body('diversityWeight').optional().isFloat({ min: 0, max: 1 }),
    body('excludeRecentlyDismissed').optional().isBoolean(),
    body('excludeRecentInteractions').optional().isBoolean(),
    body('requireMutualInterests').optional().isBoolean(),
    body('maxSocialDistance').optional().isInt({ min: 1, max: 5 }),
    body('minMutualConnections').optional().isInt({ min: 0 }),
    body('weightSocialSignals').optional().isFloat({ min: 0, max: 1 }),
  ],
  handleValidationErrors,
  async (req: Request, res: Response, next: NextFunction) => {
    try {
...
router.post(
  ...
  recommendationRateLimit,
  [
    body('followerId').isUUID().withMessage('Follower ID must be a valid UUID'),
    body('followingId').isUUID().withMessage('Following ID must be a valid UUID'),
    body('followSource').isString(),
