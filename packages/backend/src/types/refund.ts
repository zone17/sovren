...
export interface RefundAnalytics {
  ...
  topReasons: Array<{                // Top refund reasons
    reason: RefundReason;
    count: number;
    percentage: number;
  }>;
  trend: Array<{                     // Refund trend over time
    date: Date;
    count: number;
    amount: number;
  }>;
  byMethod: Record<RefundMethod, {   // Breakdown by refund method
    count: number;
    amount: number;
    averageProcessingTime: number;
  }>;
  fraudIndicators: {                 // Fraud detection metrics
...
export interface FraudFlag {
  type: 'high_frequency' | 'large_amount' | 'duplicate_request' | 'suspicious_pattern' | 'blacklisted_user';
...
export interface RefundWebhookEvent {
  type: 'refund.initiated' | 'refund.authorized' | 'refund.processing' | 'refund.completed' | 'refund.failed' | 'refund.canceled';
  refundId: string;                  // Refund ID
  transactionId: string;             // Transaction ID
  userId: string;                    // User ID
  amount: number;                    // Refund amount
